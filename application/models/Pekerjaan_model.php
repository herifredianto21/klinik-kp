...
class Pekerjaan_model extends CI_Model {
    ...
    {
    	$q = "SELECT * FROM `pekerjaans` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (`nama_pekerjaan` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `pekerjaans` (`created_at`, `nama_pekerjaan`) VALUES (NOW(), '". $this->db->escape_str($f['nama_pekerjaan']) ."');";
		} else{
			$q = "UPDATE `pekerjaans` SET `updated_at` = NOW(), `nama_pekerjaan` = '". $this->db->escape_str($f['nama_pekerjaan']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `pekerjaans` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
