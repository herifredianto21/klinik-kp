...
class Department_model extends CI_Model {
	...
	function _get($data = array())
    {
        ...
        if ($data['search']['value'] && !isset($data['all'])) {
            ...
            $q .= "WHERE (`name` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `departments` (`created_at`, `name`, `tags`, `color`) VALUES (NOW(), '". $this->db->escape_str($f['name']) ."', '[]', '#000');";
		} else{
			$q = "UPDATE `departments` SET `updated_at` = NOW(), `name` = '". $this->db->escape_str($f['name']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `departments` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
