...
class Permission_model extends CI_Model {
	...
	function _get($data = array())
    {
        ...
        if ($data['search']['value'] && !isset($data['all'])) {
            ...
            $q .= "WHERE (`name` LIKE '%". $s ."%' OR `display_name` LIKE '%". $s ."%' OR `description` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `permissions` (`created_at`, `name`, `display_name`, `description`) VALUES (NOW(), '". $this->db->escape_str($f['name']) ."', '". $this->db->escape_str($f['display_name']) ."', '". $this->db->escape_str($f['description']) ."');";
		} else{
			$q = "UPDATE `permissions` SET `updated_at` = NOW(), `name` = '". $this->db->escape_str($f['name']) ."', `display_name` = '". $this->db->escape_str($f['display_name']) ."', `description` = '". $this->db->escape_str($f['description']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `permissions` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
