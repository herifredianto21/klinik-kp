...
class Surat_hamil_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.*, b.`nama_pasien`, c.`nama_dokter` FROM `surat_hamils` a LEFT JOIN `pasiens` b ON a.`id_pasien` = b.`id` LEFT JOIN `dokters` c ON a.`id_dokter` = c.`id` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`alamat` LIKE '%". $s ."%' OR b.`nama_pasien` LIKE '%". $s ."%' OR a.`usia_kehamilan` LIKE '%". $s ."%' OR a.`dari_tanggal` LIKE '%". $s ."%' OR a.`sampai_tanggal` LIKE '%". $s ."%' OR c.`nama_dokter` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
		...
		if ($id == 0) {
            ...
            $q =    "UPDATE 
                    ...
                    SET 
                        ...
                        `usia_kehamilan` = '". $this->db->escape_str($f['usia_kehamilan']) ."', 
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `surat_hamils` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
