...
class Surat_postpartum_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.*, b.`nama_pasien`, c.`nama_dokter` FROM `surat_postpartums` a LEFT JOIN `pasiens` b ON a.`id_pasien` = b.`id` LEFT JOIN `dokters` c ON a.`id_dokter` = c.`id` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`alamat_pasien` LIKE '%". $s ."%' OR b.`nama_pasien` LIKE '%". $s ."%' OR a.`diagnosa` LIKE '%". $s ."%' OR a.`tanggal_kontrol` LIKE '%". $s ."%' OR c.`nama_dokter` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
		...
		if ($id == 0) {
			$q =    "INSERT INTO 
                    ...
                    VALUES 
                        (
                            ...
                            '". $this->db->escape_str($f['diagnosa']) ."',
                            '". $this->db->escape_str($f['tanggal_kontrol']) ."',
            ...
            $q =    "UPDATE 
                    ...
                    SET 
                        ...
                        `diagnosa` = '". $this->db->escape_str($f['diagnosa']) ."', 
                        `tanggal_kontrol` = '". $this->db->escape_str($f['tanggal_kontrol']) ."', 
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `surat_postpartums` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
