...
class Jenis_pelayanan_model extends CI_Model {
    ...
    {
    	$q = "SELECT * FROM `jenis_pelayanans` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (`nama_pelayanan` LIKE '%". $s ."%' OR `ket_pelayanan` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `jenis_pelayanans` (`created_at`, `nama_pelayanan`, `ket_pelayanan`) VALUES (NOW(), '". $this->db->escape_str($f['nama_pelayanan']) ."', '". $this->db->escape_str($f['ket_pelayanan']) ."');";
		} else{
			$q = "UPDATE `jenis_pelayanans` SET `updated_at` = NOW(), `nama_pelayanan` = '". $this->db->escape_str($f['nama_pelayanan']) ."', `ket_pelayanan` = '". $this->db->escape_str($f['ket_pelayanan']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `jenis_pelayanans` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
