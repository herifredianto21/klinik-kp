...
class Satuan_model extends CI_Model {
    ...
    {
    	$q = "SELECT * FROM `satuans` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (`nama_satuan` LIKE '%". $s ."%' OR `ket_satuan` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `satuans` (`created_at`, `nama_satuan`, `ket_satuan`) VALUES (NOW(), '". $this->db->escape_str($f['nama_satuan']) ."', '". $this->db->escape_str($f['ket_satuan']) ."');";
		} else{
			$q = "UPDATE `satuans` SET `updated_at` = NOW(), `nama_satuan` = '". $this->db->escape_str($f['nama_satuan']) ."', `ket_satuan` = '". $this->db->escape_str($f['ket_satuan']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `satuans` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
