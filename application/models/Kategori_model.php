...
class Kategori_model extends CI_Model {
    ...
    {
    	$q = "SELECT * FROM `kategoris` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (`nama_kategori` LIKE '%". $s ."%' OR `ket_kategori` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `kategoris` (`created_at`, `nama_kategori`, `ket_kategori`) VALUES (NOW(), '". $this->db->escape_str($f['nama_kategori']) ."', '". $this->db->escape_str($f['ket_kategori']) ."');";
		} else{
			$q = "UPDATE `kategoris` SET `updated_at` = NOW(), `nama_kategori` = '". $this->db->escape_str($f['nama_kategori']) ."', `ket_kategori` = '". $this->db->escape_str($f['ket_kategori']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `kategoris` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
