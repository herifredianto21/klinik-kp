...
class Data_icd_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT * FROM `data_icds` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (`data_icd` LIKE '%". $s ."%' OR `kode_icd` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `data_icds` (`created_at`, `data_icd`, `kode_icd`) VALUES (NOW(), '". $this->db->escape_str($f['data_icd']) ."', '". $this->db->escape_str($f['kode_icd']) ."');";
		} else{
			$q = "UPDATE `data_icds` SET `updated_at` = NOW(), `data_icd` = '". $this->db->escape_str($f['data_icd']) ."', `kode_icd` = '". $this->db->escape_str($f['kode_icd']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `data_icds` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
