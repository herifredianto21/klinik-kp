            	...
            	if (response.result) {
            		for(var x in response.data){
	            		...
	            		row.push({
                            ...
                            'jasa_klinik'       : response.data[x].jasa_klinik,
...
$('#tableBiayaMedis').on('click', 'button[name="btn_edit"]', function(){
	...
	$('#tableBiayaMedis tbody tr').each(function(){
		...
		if (selected == id) {
            ...
            klinik = $(this).find('td:eq(4)').text();
            ket = $(this).find('td:eq(5)').text();
		}
	});
    ...
    $('input[name="jasa_klinik"]').val(klinik);
